...
\code{MpM/mpm_solid.py} & solide : \code{P2G_solid}, \code{grid_update}, \code{G2P_solid}, endommagement non local, utilitaires \\
...
\begin{enumerate}
  ...
  \item \textbf{Endommagement} : une variable $D_p$ croît de façon irréversible quand l'allongement
        (lissé sur quelques cellules) dépasse un seuil ; la raideur devient $(1-D_p)E$. La poutre
        s'assouplit là où elle est trop sollicitée.
...
\end{enumerate}
On implémente les trois avec deux drapeaux \code{use_damage} et \code{use_rupture}, pour pouvoir les
comparer au clavier : l'élasticité vit dans \code{P2G_solid} / \code{G2P_solid}, l'endommagement et
la rupture dans deux kernels dédiés qui suivent le \code{G2P_solid} (voir \S\ref{sec:damage}).
...
\[
  D_{\rm cible} = \operatorname{clamp}\Big(\frac{\bar\varepsilon_p - \varepsilon_0}{\varepsilon_f - \varepsilon_0},\,0,\,1\Big),
  \qquad
  D_p \leftarrow \max\!\Big(D_p,\ \min\big(D_{\rm cible},\ D_p + \tfrac{\Delta t}{\tau_D}\big)\Big).
\]
Trois choses se lisent dans cette formule.
\begin{itemize}
  \item Le $\max$ rend $D_p$ \textbf{irréversible} : une fissure ne se referme pas.
  \item $\bar\varepsilon_p$ est un allongement \emph{lissé} (non local), pas celui de la particule
        seule ; voir plus bas.
  \item $D_p$ ne peut croître que de $\Delta t/\tau_D$ par pas : la raideur ne s'effondre pas
        instantanément.
\end{itemize}
La raideur effective est
\[
  E_{\rm eff} = \max(1 - D_p,\,k_{\rm res})\,E \quad\Longleftrightarrow\quad
  \mu_{\rm eff} = \max(1-D_p,\,k_{\rm res})\,\mu,\ \lambda_{\rm eff} = \max(1-D_p,\,k_{\rm res})\,\lambda,
\]
où le plancher $k_{\rm res} = 10^{-3}$ évite qu'une particule complètement endommagée n'ait plus
aucune raideur (elle se comporterait comme de la poussière).
C'est un \emph{adoucissement} : plus la particule est endommagée, moins elle résiste, donc plus elle
s'allonge, donc plus $D_p$ croît.

\paragraph{Le piège : un solide qui éclate comme du verre.} Cet adoucissement est une instabilité.
Dans la version la plus naïve (allongement de la particule seule, $D_p = D_{\rm cible}$ à chaque
pas), toutes les particules d'une zone tendue ont presque la même déformation : elles franchissent
le seuil ensemble, se déchargent ensemble, et l'énergie élastique qu'elles stockaient part en onde
de choc qui surcharge et rompt les voisines au pas suivant. On n'obtient pas une fissure mais une
poutre entière qui se pulvérise. Il manque deux ingrédients : une \emph{longueur} interne (la
largeur d'une fissure) et un \emph{temps} interne (la durée d'une rupture). Le
\S\ref{sec:shatter} chiffre l'effet de chacun.

\paragraph{Longueur interne : endommagement non local.} On remplace l'allongement $\varepsilon_p$ de la
particule par sa moyenne pondérée sur un voisinage de quelques cellules. MPM la fournit presque
gratuitement, avec la grille : un dépôt (scatter) des allongements, puis une relecture (gather),
avec les mêmes poids que d'habitude,
\[
  \hat\varepsilon_i = \frac{\sum_q w_{iq}\,\varepsilon_q}{\sum_q w_{iq}},
  \qquad
  \bar\varepsilon_p = \sum_i w_{ip}\,\hat\varepsilon_i .
\]
Le support est d'environ cinq cellules (deux passes du stencil $3\times3$). Le pic d'allongement est
étalé : l'endommagement croît sur une bande de largeur à peu près fixe, au lieu de chaque particule
indépendamment. Les particules déjà rompues sont exclues de la somme, pour qu'elles ne diluent
pas l'allongement à la pointe de la fissure. Cette largeur est fixée par la grille (elle vaut
quelques $\Delta x$) : à résolution plus fine, la fissure est plus fine.

\paragraph{Temps interne : vitesse d'endommagement limitée.} Avec $D_p \leftarrow D_p + \Delta t/\tau_D$
au plus, la raideur passe de 1 à $k_{\rm res}$ en au moins $\tau_D$ (quelques dizaines de pas ; dans
le code $\tau_D = 2\cdot10^{-3}$~s, du même ordre que le temps $h/c_p \approx 1.3\cdot10^{-3}$~s que
met une onde à traverser l'épaisseur de la poutre). Le champ de contrainte a le temps de se
réorganiser pendant que le matériau s'adoucit. Trop grand, le matériau devient visqueux et ne
casse plus ; trop petit, on retrouve la rupture en cascade.
...
\begin{enumerate}
  \item à l'instant de la rupture, on écrête \emph{immédiatement} $\mathbf F_p$ (même formule qu'au
        point suivant) : la traction disparaît, la compression éventuelle est conservée. La version
        naïve remettait $\mathbf F_p \leftarrow \mathbf I$ ; cela efface aussi l'état de compression
        (le contact) de la particule et ajoute un saut de configuration à chaque rupture
        (\S\ref{sec:shatter}, tableau) ;
...
\subsection{Éviter l'éclatement \og comme du verre \fg{}}
\label{sec:shatter}

Une première version de ce solveur cassait bien, mais trop bien : au moindre dépassement du seuil,
la poutre entière volait en éclats. Les corrections du \S\ref{sec:damage} viennent de là. Pour
savoir lesquelles comptent vraiment, on a désactivé chacune à tour de rôle sur la poutre de
\code{MpM/main.py} ($n = 256$, $E = 3\cdot10^4$, charge $70\,g$), puis mesuré, à $t \approx 0.26$~s,
après la chute et le choc contre le sol, la part de particules rompues et le nombre de morceaux
(composantes connexes de plus de 5 particules) :

\begin{center}
\begin{tabular}{@{}lrr@{}}
\toprule
Version & particules rompues & morceaux \\
\midrule
naïve (aucune des corrections ci-dessous) & 47.6~\% & 24 \\
naïve, mais avec rampe de charge et amortissement & 21.3~\% & 6 \\
sans rampe de charge ni amortissement & 22.1~\% & 14 \\
sans lissage non local & 14.1~\% & 4 \\
sans limite de vitesse ($\tau_D \to 0$) & 10.6~\% & 14 \\
avec $\mathbf F \leftarrow \mathbf I$ à la rupture & 4.1~\% & 5 \\
sans raideur résiduelle $k_{\rm res}$ & 2.6~\% & 4 \\
\textbf{version du tutoriel (tout activé)} & \textbf{2.5~\%} & \textbf{4} \\
\bottomrule
\end{tabular}
\end{center}

Une seule simulation par ligne : ce sont des ordres de grandeur, pas des statistiques. On en tire :
\begin{itemize}
  \item \textbf{Trois causes se dégagent} : l'énergie cinétique injectée d'un coup
        (charge appliquée instantanément, sans amortissement), l'absence de longueur interne
        (sans lissage non local) et l'absence de temps interne (sans limite de vitesse).
        Chacune, retirée seule, multiplie par 4 à 9 la part de particules rompues.
  \item Le \code{F = I} à la rupture et le plancher $k_{\rm res}$ ont un effet faible, mais le
        premier est pénalisant et le second ne coûte rien : on les garde.
  \item Le choc contre le sol casse toujours \emph{quelque chose}. La vitesse d'impact $v$ est
        d'autant plus grande qu'on a dû charger la poutre pour la casser, et l'allongement produit
        est de l'ordre de $v/c_p$ ; la charge nécessaire pour casser croît comme $E$, donc
        (estimation, non mesurée) ce rapport varie peu avec $E$. Il ne s'agit donc pas de supprimer toute rupture au choc, seulement d'éviter
        qu'elle ne se propage de proche en proche à toute la poutre.
\end{itemize}

Réglages pratiques : $\tau_D$ de l'ordre du temps de traversée de l'épaisseur ; charge en rampe
plutôt qu'appliquée d'un coup (surtout au-delà de la centaine de $g$) ; un amortissement de grille
de quelques dizaines de s$^{-1}$, suffisant pour dissiper l'énergie des fragments sans écraser la
dynamique.
...
$\tau_D$ & $2\cdot10^{-3}$~s & durée minimale du passage de $D=0$ à $D=1$ \\
$k_{\rm res}$ & $10^{-3}$ & raideur résiduelle $(1-D \ge k_{\rm res})$ \\
amortissement & 20~s$^{-1}$ & fraction de vitesse de grille retirée par seconde \\
rampe de charge & 50~$g$/s & la charge monte progressivement jusqu'à la valeur du curseur \\
...
multiplie $g$ (de 0 à 12) ; comme l'allongement est lissé, la rupture n'apparaît que vers une
charge de 8.
...
grid_w = ti.field(ti.f32, (n_grid, n_grid))    # poids associés
\end{lstlisting}

Les deux champs \code{grid_e} et \code{grid_w} ne servent qu'à l'endommagement non local (étape 5).
...
def P2G_solid(grid_m: ti.template(), grid_v: ti.template(),
    ...
    for p in x:
        ...
        k = ti.max(1.0 - D[p], k_res)  # raideur effective (1 - D) E, avec un plancher
        if broken[p] == 1:
...
\begin{itemize}
  \item \code{k} : le facteur $(1-D_p)$ du \S\ref{sec:damage}, borné par le plancher \code{k_res},
        forcé à 1 pour une particule rompue (\S\ref{sec:rupture}, point 3).
...
La mise à jour de la grille est \emph{presque identique} à celle du fluide (un terme
d'amortissement en plus, voir plus bas) : on la recopie dans
...
def grid_update(grid_m: ti.template(), grid_v: ti.template(), mask: ti.template(),
    ...
    for i, j in grid_m:
        if grid_m[i, j] > 0:
            ...
            grid_v[i, j] *= 1.0 - damp * dt         # amortissement léger
            grid_v[i, j].y -= dt * g
...
\paragraph{Amortissement.} La ligne \code{grid_v[i, j] *= 1.0 - damp * dt} retire à chaque pas une
fraction \code{damp * dt} de la vitesse de grille (\code{damp} $= 20$~s$^{-1}$). C'est une
dissipation numérique volontaire : elle absorbe l'énergie cinétique des fragments et évite qu'ils
ne continuent de vibrer et de se rompre. En partie II, le fluide utilise \code{grid_step}, sans
ce terme, et le solide n'en a donc pas non plus.
...
le bloc \code{if broken} est expliqué à l'étape 6 et est inactif en mode élastique.
L'endommagement n'y figure plus : il a besoin d'un lissage sur la grille et fait l'objet de
kernels séparés (étape 5).
...
def G2P_solid(grid_v: ti.template(),
    ...
    for p in x:
        ...
        if broken[p] == 1:                          # étape 6 : particule rompue
...
les vibrations). Avec le curseur de charge à 3, la flèche est multipliée par 2.5, sans jamais casser : un solide
...
L'endommagement est calculé \emph{après} le \code{G2P_solid}, en trois kernels qui suivent la loi du
\S\ref{sec:damage} : remise à zéro de la grille de lissage, dépôt des allongements, puis relecture
et mise à jour de $D_p$. On abrège les arguments \code{ti.template()} :

\begin{lstlisting}
@ti.kernel
def clear_eps(grid_e, grid_w):
    for i, j in grid_e:
        grid_e[i, j] = 0.0
        grid_w[i, j] = 0.0

@ti.kernel
def scatter_eps(grid_e, grid_w, x, F, broken, inv_dx: float):
    for p in x:
        if broken[p] == 0:                              # les particules rompues ne comptent pas
            base = (x[p] * inv_dx - 0.5).cast(int)
            fx = x[p] * inv_dx - base
            w = [0.5 * (1.5 - fx)**2, 0.75 - (fx - 1.0)**2, 0.5 * (fx - 0.5)**2]
            U, sig, V = ti.svd(F[p])
            eps = ti.max(sig[0, 0], sig[1, 1]) - 1.0    # allongement principal maximal
            for i, j in ti.static(ti.ndrange(3, 3)):
                weight = w[i].x * w[j].y
...
@ti.kernel
def update_damage(grid_e, grid_w, x, F, D, broken, inv_dx: float, dt: float,
    ...
    for p in x:
        if broken[p] == 0:
            ...
            fx = x[p] * inv_dx - base
            w = [0.5 * (1.5 - fx)**2, 0.75 - (fx - 1.0)**2, 0.5 * (fx - 0.5)**2]
            eps = 0.0                                   # allongement lissé (gather)
            for i, j in ti.static(ti.ndrange(3, 3)):
...
\paragraph{Ligne par ligne.}
\begin{itemize}
  \item \code{scatter_eps} : même stencil et mêmes poids que le P2G ; la grille reçoit
        $\sum_q w_{iq}\varepsilon_q$ dans \code{grid_e} et $\sum_q w_{iq}$ dans \code{grid_w}. On
        ne pondère pas par la masse : toutes les particules solides ont la même.
  \item \code{update_damage} : le \code{gather} relit $\hat\varepsilon_i = \code{grid_e}/\code{grid_w}$
        aux 9 n\oe uds et le repondère, ce qui donne $\bar\varepsilon_p$. Le test
        \code{grid_w[node] > 0} évite une division par zéro (cas impossible pour un n\oe ud du
        stencil d'une particule saine, mais le garde-fou ne coûte rien).
  \item \code{D_new} : interpolation linéaire entre $\varepsilon_0$ et $\varepsilon_f$, bornée à $[0,1]$.
  \item \code{ti.max(D[p], ti.min(D_new, D[p] + dt / tau_D))} : irréversibilité \emph{et} vitesse
        limitée, en une ligne.
\end{itemize}
Et $D_p$ est consommé dans \code{P2G_solid} via \code{k}. On ne passe par ces kernels qu'en modes 2
et 3 (\code{if use_damage == 1} dans \code{substep}, voir l'étape 7). En mode 1, \code{D} reste nul.

\paragraph{Ce que l'on doit voir (touche 2).} La charge monte en rampe jusqu'à la valeur du
curseur. À charge 3, $D_{\max}$ reste voisin de $0.05$ : rien ne bouge. À charge 4, $D_{\max}$
atteint $0.16$, à charge 6 environ $0.35$ et à charge 7 environ $0.5$ : les particules jaunissent
et la flèche augmente (de $0.02$ à $0.045$ entre les charges 3 et 7), mais la poutre tient. Ramenez la
charge à 1 : la flèche diminue mais la couleur reste, $D$ ne redescend jamais. À charge 8 ou plus,
$D_{\max}$ atteint 1 et toute la poutre s'effondre d'un bloc sans se séparer : les particules de
raideur quasi nulle restent \og accrochées \fg{} par la grille. C'est le problème annoncé au
\S\ref{sec:rupture}, et la raison de l'étape suivante.

% ================================================================
\section{Étape 6 : rupture}

Deux morceaux de code. D'abord la \emph{transition}, à la fin de \code{update_damage} (là où l'étape 5
avait mis un commentaire) :
\begin{lstlisting}
            if use_rupture == 1 and D[p] >= 1.0:
                broken[p] = 1
                U, sig, V = ti.svd(F[p])            # on retire la traction tout de suite
                for d in ti.static(range(2)):
                    sig[d, d] = ti.math.clamp(sig[d, d], 0.1, 1.0)
                F[p] = U @ sig @ V.transpose()
\end{lstlisting}
Ensuite le comportement \emph{permanent} d'une particule rompue, en tête du \code{if} de \code{G2P_solid} :
\begin{lstlisting}
        if broken[p] == 1:
            U, sig, V = ti.svd(F_new)
            for d in ti.static(range(2)):
...
rotation. Une particule rompue ne repasse jamais par \code{update_damage} (test
\code{broken[p] == 0}).

\paragraph{Ce que l'on doit voir (touche 3).} Chargez progressivement. Jusqu'à 7, rien ne
casse. À 8, la première particule rompue apparaît vers $t \approx 0.18$~s près d'un encastrement,
sur la fibre supérieure : c'est là que le moment fléchissant d'une poutre bi-encastrée est maximal
($qL^2/12$ contre $qL^2/24$ à mi-portée). La poutre se détache des piliers et tombe. Au bout d'une
seconde, seules 60 particules sur 2685 (2~\%) sont rompues, choc contre le sol compris : la poutre
se rompt, elle n'éclate pas (comparez avec le \S\ref{sec:shatter}).

Mesures faites sur ce code, avec les paramètres de la table du \S\ref{sec:stab} (charge en rampe de
50~$g$/s), sur 1~s ; la flèche est la moyenne de $0.58 - y$ sur une bande de $\pm0.03$ autour de
mi-portée :
...
\midrule
1 & 3 & 0 & flèche $\approx 0.01$, aucune particule endommagée \\
4 & 3 & 0.16 & flèche $\approx 0.03$, la poutre tient \\
6 & 3 & 0.35 & flèche $\approx 0.04$, la poutre tient \\
7 & 3 & 0.48 & flèche $\approx 0.045$, la poutre tient \\
8 & 3 & 1 & rupture près de l'encastrement à $t\approx0.18$~s, 60 particules rompues à 1~s, chute \\
3 & 1 & -- & (élastique) flèche $\approx 0.023$, ne casse pas \\
\bottomrule
\end{tabular}
\end{center}
...
\paragraph{Rampe de charge.} Le curseur fixe une charge \emph{cible} \code{load_target} ; la charge
effective \code{load} part de 0 et la rejoint à \code{ramp_rate} $= 50\,g$/s :
\begin{lstlisting}
            load = min(load + ramp_rate * dt, load_target) if load < load_target else load_target
            substep(load, use_damage, use_rupture)
\end{lstlisting}
Une charge appliquée d'un coup injecte de l'énergie cinétique qui fait éclater la poutre
(\S\ref{sec:shatter}) ; la touche R remet \code{load} à 0.
...
\begin{itemize}
  ...
  \item Retrouvez l'éclatement : mettez \code{tau_D = 1e-9} (plus de limite de vitesse), ou
        \code{ramp_rate = 1e9} et \code{damp = 0} (charge instantanée), et comparez la couleur
        \og endommagement \fg{} à charge 8 : la rupture se propage à toute la poutre.
...
\begin{tikzpicture}[
  ...
  \node[stp,fill=orange!10, left=0.5cm of d] (e) {5. \code{G2P_solid}\\ $v, C, \mathbf F$, advection};
...
\begin{itemize}
  ...
  \item \code{grid_step} du fluide sert tel quel : son masque \code{solid} (obstacles) devient le
        masque des piliers.
  \item L'endommagement (\code{clear_eps}, \code{scatter_eps}, \code{update_damage}) s'exécute juste
        après le \code{G2P_solid}. Il utilise ses propres champs de grille \code{grid_e} et
        \code{grid_w} : seules les particules solides y contribuent, le fluide n'est pas lissé avec
        le solide.
...
from MpM.mpm_solid import (P2G_solid, G2P_solid, clear_eps, scatter_eps, update_damage,
...
C'est le c\oe ur de la partie II, et il tient en sept étapes dans l'ordre du schéma (l'endommagement,
qui n'existe pas côté fluide, se glisse entre le \code{G2P_solid} et l'advection du fluide) :
...
\paragraph{L'endommagement dans le couplage.} Les étapes 6 relisent l'allongement de chaque
particule solide, le lissent sur la grille (\code{grid_e}, \code{grid_w}, propres au solide) et mettent
à jour $D_p$ à vitesse limitée ; l'eau n'intervient qu'à travers la vitesse nodale commune, donc à
travers $\mathbf F_p$. La grille commune \code{grid_m} n'est pas modifiée par ces trois appels.
...
(en mode déformation : rouge dessous à mi-portée, rouge dessus aux piliers), jaunit, puis entre
$0.2$ et $0.3$~s les premières particules rouges apparaissent aux encastrements. Le tablier se
...
\midrule
0.10 & 0 & 0 & 0.417 & chute libre de l'eau, tablier au repos sous son poids \\
0.20 & 0.05 & 0 & 0.397 & impact, flexion, début de l'endommagement \\
0.30 & 1 & 61 & 0.298 & rupture aux deux encastrements, le tablier tombe \\
0.50 & 1 & 64 & 0.242 & tablier dans le bassin \\
1.00 & 1 & 64 & 0.233 & tablier au fond, l'eau se calme \\
\bottomrule
\end{tabular}
\end{center}

La masse totale sur la grille (fluide $+$ solide) est conservée à la précision machine, $J_f$
reste dans $[0.46, 1.51]$ (les valeurs extrêmes sont des particules d'eau coincées entre le
...
\begin{itemize}
  \item Rendez le tablier plus résistant ($E_s = 10^4$ ou $\varepsilon_f = 0.4$) : il encaisse la
        vague et vibre. Plus fragile ($\varepsilon_f = 0.08$) : la rupture est plus précoce et plus nette.
...
def P2G_solid(grid_m: ti.template(), grid_v: ti.template(),
    ...
    for p in x:
        ...
        k = ti.max(1.0 - D[p], k_res)      # plancher : jamais totalement sans raideur
        if broken[p] == 1:
            k = 1.0
        tau = kirchhoff_stress(F[p], k * mu, k * la)
...
def update_damage(grid_e: ti.template(), grid_w: ti.template(),
                  ...
                  eps0: float, epsf: float, tau_D: float, use_rupture: int):
    """Endommagement non local (allongement lissé) et à vitesse limitée (dD <= dt / tau_D)."""
    for p in x:
        if broken[p] == 0:
            base = (x[p] * inv_dx - 0.5).cast(int)
            fx = x[p] * inv_dx - base
...
tau_D = 2e-3                         # D croît au plus de dt/tau_D par pas
k_res = 1e-3                         # raideur résiduelle (1-D >= k_res)
damp = 20.0                          # amortissement de la vitesse de grille (1/s)
ramp_rate = 50.0                     # montée de la charge (x g par seconde simulée)
...
grid_v = ti.Vector.field(2, ti.f32, (n_grid, n_grid))
grid_m = ti.field(ti.f32, (n_grid, n_grid))
grid_e = ti.field(ti.f32, (n_grid, n_grid))
grid_w = ti.field(ti.f32, (n_grid, n_grid))
...
def main():
    ...
    while window.running:
        ...
        load_target = gui.slider_float("charge (x g)", load_target, 0.0, 12.0)
